...
@section('title', 'Edit ' . $cafe->nama_cafe)

@section('content')
    <div class="max-w-2xl mx-auto animate-fade-in-up">
        {{-- Header --}}
        <div class="mb-8">
            <a href="{{ route('cafe.index') }}" class="inline-flex items-center gap-2 text-sm font-bold text-black border-b-2 border-transparent hover:border-black transition-colors mb-4 uppercase" id="btn-back-edit">
            ...
            <h1 class="text-3xl font-retro text-black uppercase text-shadow-[3px_3px_0px_#3b82f6]">Edit <span class="bg-[#fbbf24] px-2 border-2 border-black inline-block mt-2">{{ $cafe->nama_cafe }}</span></h1>
            <p class="text-black font-bold mt-4 bg-white inline-block px-2 border-2 border-black">Perbarui data cafe dan nilai sub-kriteria</p>
        </div>

        {{-- Form --}}
        <form action="{{ route('cafe.update', $cafe) }}" method="POST" class="bg-white border-4 border-black p-6 sm:p-8 space-y-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]" id="form-edit-cafe">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white border-4 border-black p-5 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]" id="edit-field-{{ $name }}">
                    ...
                    @php $currentVal = number_format((float) old($name, $cafe->{$name}), 1); @endphp
                    <div class="flex items-center justify-between mb-4 border-2 border-dashed border-black p-2 bg-[#f8fafc]">
                        <span class="text-3xl font-retro text-[#2563eb]" id="val-{{ $name }}">{{ $currentVal }}</span>
                        <span class="text-sm font-bold text-black uppercase" id="label-{{ $name }}">{{ $field['scale'][$currentVal] ?? '' }}</span>
                    </div>
            ...
            <div class="flex flex-col sm:flex-row items-center gap-4 pt-6 border-t-4 border-black">
                <button type="submit" class="btn-primary w-full sm:w-auto text-base" id="btn-submit-edit">
                ...
                <a href="{{ route('cafe.index') }}" class="btn-secondary w-full sm:w-auto text-center" id="btn-cancel-edit">BATAL</a>
            </div>
        </form>
    </div>
@endsection
